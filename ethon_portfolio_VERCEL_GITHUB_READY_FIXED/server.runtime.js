...
// Vercel's deployed bundle is read-only. All mutable CMS/runtime files must live in
// /tmp there; the checked-in data.json remains the read-only seed/default state.
const WRITABLE_ROOT = process.env.VERCEL ? path.join('/tmp', 'ethon-cms') : ROOT;
const DATA_FILE = path.join(WRITABLE_ROOT, 'data.json');
const UPLOAD_DIR = path.join(WRITABLE_ROOT, 'uploads');
const CHAT_UPLOAD_DIR = path.join(WRITABLE_ROOT, 'chat-uploads');
const RUNTIME_DIR = path.join(WRITABLE_ROOT, 'runtime');
const SEED_DATA_FILE = path.join(ROOT, 'data.json');
const PORT = Number(process.env.PORT) || 3000;
const SESSION_TTL = 28800; // 8 hours

if (!fs.existsSync(WRITABLE_ROOT)) fs.mkdirSync(WRITABLE_ROOT, { recursive: true });
if (process.env.VERCEL && !fs.existsSync(DATA_FILE) && fs.existsSync(SEED_DATA_FILE)) {
